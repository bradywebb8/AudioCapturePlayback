...
//
//  Created by Brady Webb on 11/1/19.
...
class AudioViewController: UIViewController, AVAudioPlayerDelegate, AVAudioRecorderDelegate {
    ...
    override func viewDidLoad()
    {
        ...
        audioSession?.requestRecordPermission()
            ...
            {
                self.presentAlert(message: "Denied.")
            }
        }
    ...
    func initializeAudioRecorder()
    {
        ...
        {
            presentAlert(message: "No audio file.")
            return
        }
        ...
        {
            presentAlert(message: "Error \(error.localizedDescription)")
        }
    }
    ...
    @IBAction func playButtonTapped(_ sender: Any)
    {
        guard let audioFileURL = audioFileURL else
        {
            presentAlert(message: "Not available to play.")
            return
        }
        
        guard let audioRecorder = audioRecorder, audioRecorder.isRecording == false else
        {
            presentAlert(message: "Unable to play.")
            return
        }
